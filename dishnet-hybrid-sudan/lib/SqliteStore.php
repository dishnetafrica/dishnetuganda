...
class SqliteStore implements StoreInterface
{
    ...
    public static function create(string $dataDir): self
    {
        ...
        // ── Seed BlueCard data on first boot ─────────────────────────────
        // Copies lte_*.json from data/seed/ into the store if tables are empty.
    ...
    public function firstBootMigration(): void
    {
        ...
        foreach (glob($this->dir . '/*.json') as $jsonPath) {
            ...
            // config.json and ucrm.json belong to uCRM, not to us. uCRM
            // rewrites config.json every time an admin saves the plugin
            // settings form, so importing and renaming it would silently
            // discard the next settings change.
