...
| | **Status 2026-09-23 (`docs/118`):** register and assign **bound**; the action **not bound** (needs an Admin-plane enqueue function, a migration; 501 says so); matrix, staff-username audit, manifest equality and the worker-level *queued → retryable* proof all met; `declared_unbound` is now six (action, sites, plans, voucher-batches, disconnect, principals). No hardware claim. |
| | **Status 2026-09-23, later (`docs/121`):** the action is **bound** by migration 028 — `mt_device_provision_request` (owner `dnb_def_prov`, EXECUTE `dnb_adminwrite` only), `push_config` queues a `device.provision` intent and every other action answers 501 with the inventory's reason; a **lifecycle route** (`POST /routers/{id}/state`, new capability `routers.lifecycle`, Admin and NOC) records the state a person observed, with `mt_device_set_state` now a RULE I-1 no-op for a state the row already holds; `declared_unbound` is five; the *queued → retryable* proof runs from the route. Panel forms for register, assign, lifecycle and push are built. No hardware claim. |
...
| | **Status 2026-09-23 (`docs/122`):** rehearsed on **STAGING** by one command — the real provider behind Traefik's TLS with `DN_TRUSTED_PROXY` **proved by the script's own sign-in attempt through Traefik** (corrected at most once, else rolled back), `DN_PORTAL_ORIGIN` set, doctor in production posture with 0 blockers, the stop-gap basic auth removed only after the provider is live, `staff:bootstrap` only after the proof, TOTP required. Harness: 108/108. **SWITCHED 2026-09-23 21:41:56 UTC**: proved from `172.22.0.1` on the first attempt, 0 blockers; the widening applies, so G-E must give the proxy an address of its own (`docs/122` §E.1). Not production; no backups, no disposable-VM rehearsal from the tarball. |
