...
| **Failure behaviour** | A refused redemption — unknown, spent, revoked, expired, wrong site, unknown NAS, rate-limited — **writes no `mt_audit_log` row**. **CORRECTED by `docs/88` §C.2:** it is *not* unrecorded. `docs/65` §8 (SETTLED) requires failed attempts in a **separate non-tenant attempt store**, classified by outcome, with `code_prefix` + `code_hash` and never the raw code — and `docs/65` §9.5 puts Decision 5's counters in that same store, making it a **prerequisite**, not an extra. It is not an act that happened, and a row would make the audit log a code oracle for anyone who can read it (`docs/65` §8, `docs/67` §4.2). A **permanent publication failure** does audit, because by then a real transition occurred. |
...
| **P-3** | Decision 5's numbers — packet in `docs/88` §A. **One premise is refuted:** `docs/68` §2.2 showed the portal's NAS is guest-asserted, so `docs/65` §9.3's primary dimension is evadable, while §9.2 records that per-code does not bound enumeration and §9.3 that source IP is not primary. Decision 5 needs that resolved (`docs/88` §A.0) before its numbers mean anything. |
