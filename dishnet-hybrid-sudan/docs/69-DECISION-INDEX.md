...
| **The next gate** | **A production census** — `n10_backfill_census.php` run against the production control plane by someone who can reach it (docs/74 §9 item 1); this session cannot. Then the N10 migration. **Q2** still blocks the `dnb_site_nas` key. **Not F6 implementation** |
