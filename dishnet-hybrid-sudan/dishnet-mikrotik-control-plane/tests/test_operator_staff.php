...
// ===========================================================================
t('15. REPOSITORY STATE — 027 is applied; 028 (docs/121) and 029 (O-1, docs/124) follow it; the ledger agrees; the census gained its line');
...
is_(substr(end($files), 0, 3), '029', 'the last migration file is 029 — O-1, a site and its service share one operator (docs/124), not a T-2 change');
is_((int) $ins->one('SELECT count(*)::int AS n FROM mt_migrations')['n'], 29, 'the ledger records 29 migrations');
is_(substr($ins->one('SELECT max(filename) AS f FROM mt_migrations')['f'], 0, 3), '029', 'the latest applied is 029');
