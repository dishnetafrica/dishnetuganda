...
// ===========================================================================
t('15. REPOSITORY STATE — 027 is applied; 028 (docs/121), 029 (O-1, docs/124), 030 (onboarding, docs/125), 031 and 032 (sign-in, docs/127) follow it; the ledger agrees; the census gained its line');
...
is_(substr(end($files), 0, 3), '032', 'the last migration file is 032 — sign-in codes by SMS (docs/127), not a T-2 change');
is_((int) $ins->one('SELECT count(*)::int AS n FROM mt_migrations')['n'], 32, 'the ledger records 32 migrations');
is_(substr($ins->one('SELECT max(filename) AS f FROM mt_migrations')['f'], 0, 3), '032', 'the latest applied is 032');
