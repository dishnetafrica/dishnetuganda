...
// ===========================================================================
t('15. REPOSITORY STATE — 027 is applied; 028 (docs/121), 029 (O-1, docs/124), 030 (onboarding, docs/125), 031 and 032 (sign-in, docs/127) and 033 (SMS settings, docs/128) follow it; the ledger agrees; the census gained its line');
...
is_(substr(end($files), 0, 3), '033', 'the last migration file is 033 — SMS settings from the Admin panel (docs/128), not a T-2 change');
is_((int) $ins->one('SELECT count(*)::int AS n FROM mt_migrations')['n'], 33, 'the ledger records 33 migrations');
is_(substr($ins->one('SELECT max(filename) AS f FROM mt_migrations')['f'], 0, 3), '033', 'the latest applied is 033');
