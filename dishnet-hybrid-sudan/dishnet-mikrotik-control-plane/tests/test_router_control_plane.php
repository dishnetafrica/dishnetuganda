...
// ===========================================================================
t('13. REPOSITORY STATE — migrations end at 033 (SMS settings, docs/128) after 028 (docs/121), 029 (O-1, docs/124), 030 (onboarding, docs/125), 031 and 032 (sign-in, docs/127); the gate variable is declared; nothing claims hardware');
$files = array_map('basename', glob(__DIR__ . '/../migrations/*.sql')); sort($files);
is_([substr(end($files), 0, 3), count(array_filter($files, fn($f) => str_starts_with($f, '028')))], ['033', 1], 'the last migration is 033 (SMS settings, docs/128), after one 028 (docs/121); G-C itself added none');
is_((int) $ins->one('SELECT count(*)::int AS n FROM mt_migrations')['n'], 33, 'the ledger records 33');
