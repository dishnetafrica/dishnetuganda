...
3. **C-2 — approved 26 Sep, guarded.** The change is made by hand in uCRM's settings. A READ-ONLY guard,
   ...
   **21:57 — after the upload.**
   ...
   - **After 22:01, the operator's confirmation.** *"yes thi si now set correct it will creat next invoice as you
     said"*: the organization's invoice template is "v2". This is recorded as the operator's statement; no
     screenshot came. **The one remaining proof is the next invoice for client #1** (Family Shoppers): run
     `--links` after uCRM issues it.
