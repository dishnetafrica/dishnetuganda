...
| deployment / configuration / customer data | **5.18.41 and 5.18.42 deployed by the operator; one Traefik file placed by C-1 attempt 1 and ignored by Traefik, rewritten by attempt 2 and taken; no configuration value, customer record or other service changed** | — |
...
**Next: deploy 5.18.43 (one label on the invoice screen). It is small; run it when convenient.** The first
