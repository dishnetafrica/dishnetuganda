...
check "$(printf '%s\n' "$out" | grep -c 'staged outside the watched directory')" "1" "S1 the file was staged outside the watched directory and moved in one step"
check "$(ls "$SB/conf" | grep -c 'tmp')" "0" "S1 no temporary file was ever left in the watched directory"
