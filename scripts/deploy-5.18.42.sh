#!/usr/bin/env bash
#
# deploy-5.18.42.sh — deploy plugin 5.18.42: change set A2 of docs/38 and the invoice's tax lines
# (built on the operator's "go with your recommendation" of 26 September 2026, docs/38 §7.3 / §7.5).
...
#
# Same machinery as deploy-5.18.41.sh: before-evidence, a backup, the documented deploy
...
# backup under /root/dnb-5.18.42/ — and, only if stage V finds the public address redirecting,
...
EXPECTED_PLUGIN_COMMIT="d857ec8"   # 5.18.42 — the plugin-scoped commit deploy-hybrid.sh records
EXPECTED_VERSION="5.18.42"
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$REPO/$PLUGIN"
TS="$(date -u +%Y%m%dT%H%M%SZ)"
OUT="/root/dnb-5.18.42"; mkdir -p "$OUT"; chmod 700 "$OUT"
...
echo "  before: the Terms page → $HTTP_CODE; 'South Sudan' ×$(count 'South Sudan') · Juba ×$(count 'Juba') · 'registered in Uganda' ×$(count 'registered in Uganda')  (A2 turns the first two to 0)"
http GET "$PLUGIN_BASE?page=api&action=app_legal_version"
echo "  before: app_legal_version → $HTTP_CODE $(printf '%s' "$HTTP_BODY" | grep -o '"tos_version":"[^"]*"' | head -1)  (5.18.42 answers 1.1 on the Uganda install)"
...
echo "  next (read-only)  docker exec ucrm php /data/ucrm/data/plugins/dishnet-hybrid-sudan/tools/tax_probe.php   → section 5: the tax lines the invoice screen prints"
