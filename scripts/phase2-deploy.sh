#!/usr/bin/env bash
#
# phase2-deploy.sh — deploy plugin 5.18.38 (customer-login audit, Phase 2: authentication)
# with before-evidence, a backup, the documented deploy, the Phase 2 posture checks and
# the safe smoke tests. Derived from phase1-deploy.sh; the machinery is the same.
#
...
#     && mkdir -p /root/dnb-phase2 \
#     && bash scripts/phase2-deploy.sh 2>&1 | tee /root/dnb-phase2/deploy-$(date -u +%Y%m%dT%H%M%SZ).log
#
# Options
#   --after-only         the deploy already happened: run only the checks
#   --webhook-only       the read-only webhook inspection alone (stage D), nothing else
#   --login-only         with --login: the customer sign-in flow alone (page, code, cookie,
#                        consent, portal, logout, no code in any log), then stage D
#   --otp-history        read-only COUNTS of the customer sign-in history; then exit
...
# post a payment, change a customer record, print a token, code, key or secret.
# The only writes are the documented deploy (scripts/deploy-hybrid.sh) and a backup
# under /root/dnb-phase2/. Every read of the plugin's database works on a COPY,
# as the database's owner, so the live file never gains a root-owned sidecar.
# The ONE configuration change it may ask for — and never makes itself — is
# tools/set_config.php --key tenant_profile --value uganda, if stage P finds the
# profile resolving to the wrong country.
#
# Stages:  A before-evidence + backup → GO/NO-GO   B the documented deploy
#          C smoke tests (HTTP against the live plugin)   P the Phase 2 posture
#          D webhook, read-only   E receipt/delivery links   F summary
set -uo pipefail
umask 077

PLUGIN="dishnet-hybrid-sudan"
CONTAINER="${UCRM_CONTAINER:-ucrm}"
EXPECTED_PLUGIN_COMMIT="fa2d463"   # 5.18.38 — the plugin-scoped commit deploy-hybrid.sh records
EXPECTED_VERSION="5.18.38"
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$REPO/$PLUGIN"
TS="$(date -u +%Y%m%dT%H%M%SZ)"
OUT="/root/dnb-phase2"; mkdir -p "$OUT"; chmod 700 "$OUT"
...
http() {
  ...
  local args=(-sS -o "/tmp/dnb_body.$$" -D "/tmp/dnb_hdr.$$" -w '%{http_code}' -X "$m" -H 'Content-Type: application/json')
  [ -n "$b" ] && args+=(--data "$b")
  local r
...
hdr "Phase 2 (5.18.38) — $TS — $(hostname)"
...
# ═════════════════════════════════════════════════════════════════════════════
hdr "P. Phase 2 posture — read-only (store copy; key NAMES only; no value, number or code is printed)"
# ═════════════════════════════════════════════════════════════════════════════
# C1 loaded the login page, which is the moment the plugin provisions the customer key set.
docker exec -u "$DB_OWNER" "$CONTAINER" sh -c "mkdir -p '$RO' && cp '$DB_IN' '$RO/db5.sqlite3' && ( [ -f '$DB_IN-wal' ] && cp '$DB_IN-wal' '$RO/db5.sqlite3-wal' || true )" 2>/dev/null || true
cat > "$SNIP/posture.php" <<'PHP'
<?php
// One fact per line, key=value. Never a secret, never a phone number.
$db = new PDO('sqlite:' . getenv('RO_DB5'), null, null, [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
$mig = $db->query("SELECT filename FROM _migrations WHERE filename LIKE '073%' OR filename LIKE '074%' ORDER BY filename")->fetchAll(PDO::FETCH_COLUMN);
echo "migrations=", implode(',', $mig), "\n";
$tables = $db->query("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('customer_sessions','client_search_index')")->fetchAll(PDO::FETCH_COLUMN);
echo "tables=", implode(',', $tables), "\n";
$cols = array_column($db->query("PRAGMA table_info(client_search_index)")->fetchAll(PDO::FETCH_ASSOC), 'name');
echo "index_flags=", (count(array_intersect(['email','is_lead','is_archived','has_service'], $cols)) === 4 ? 'yes' : 'NO'), "\n";
echo "index_rows=", (int)$db->query("SELECT COUNT(*) FROM client_search_index")->fetchColumn(), "\n";
echo "index_rows_synced=", (int)@$db->query("SELECT COUNT(*) FROM client_search_index WHERE is_lead IS NOT NULL")->fetchColumn(), "\n";
$cfg = json_decode((string)$db->query("SELECT data FROM kyc_config LIMIT 1")->fetchColumn(), true) ?: [];
require_once getcwd() . '/lib/CustomerJwtKeys.php';
echo "key_provisioned=", CustomerJwtKeys::provisioned($cfg) ? 'yes' : 'NO', "\n";
echo "active_kid=", CustomerJwtKeys::activeKid($cfg) ?: '-', "\n";
echo "key_count=", count(CustomerJwtKeys::keys($cfg)), "\n";
require_once getcwd() . '/lib/TenantProfile.php';
$t = TenantProfile::current($cfg, (string)getenv('PDD_IN'));
echo "profile=", $t->id(), "\n", "dial=", $t->dialCode(), "\n";
echo "selector=", (($cfg['tenant_profile'] ?? '') !== '' ? $cfg['tenant_profile'] : '(derived)'), "\n";
echo "currency=", (($cfg['currency_code'] ?? '') !== '' ? $cfg['currency_code'] : '(not in the store; vault or none)'), "\n";
echo "allow_leads=", (($cfg['portal_login_allow_leads'] ?? '') !== '' ? $cfg['portal_login_allow_leads'] : '(default:no)'), "\n";
echo "require_service=", (($cfg['portal_login_require_service'] ?? '') !== '' ? $cfg['portal_login_require_service'] : '(default:no)'), "\n";
echo "ttl_days=", (($cfg['app_jwt_ttl_days'] ?? '') !== '' ? $cfg['app_jwt_ttl_days'] : '(default:30)'), "\n";
echo "sessions_live=", (int)@$db->query("SELECT COUNT(*) FROM customer_sessions WHERE revoked_at IS NULL AND expires_at > " . time())->fetchColumn(), "\n";
PHP
POSTURE="$(docker exec -i -u "$DB_OWNER" -w "$IN_CONTAINER" -e "RO_DB5=$RO/db5.sqlite3" -e "PDD_IN=$PDD_IN" "$CONTAINER" php < "$SNIP/posture.php" 2>&1 || true)"
pf() { printf '%s\n' "$POSTURE" | grep "^$1=" | head -1 | cut -d= -f2-; }
case "$(pf migrations)" in *073_customer_sessions.sql*074_client_search_index_flags.sql*) ok "P migrations 073 and 074 are recorded";; *) bad "P migrations recorded: '$(pf migrations)' — expected 073 and 074 (did the container open the store since the deploy?)";; esac
case "$(pf tables)" in *customer_sessions*) ok "P customer_sessions exists";; *) bad "P customer_sessions is missing";; esac
[ "$(pf index_flags)" = "yes" ] && ok "P the customer index carries the e-mail and eligibility columns (rows $(pf index_rows), synced with flags $(pf index_rows_synced) — cron_sync fills them within minutes)" || bad "P the customer index lacks the Phase 2 columns"
[ "$(pf key_provisioned)" = "yes" ] && ok "P the customer signing key set is provisioned (active $(pf active_kid), $(pf key_count) key(s); values never read)" || bad "P no customer signing key set — C1 should have provisioned it at the first page load"
VAULT="$MOUNT/ucrm/data/plugins/.dishnet-sudan.vault.json"
if [ -f "$VAULT" ]; then n=0; for k in customer_jwt_keys customer_jwt_active_kid customer_jwt_key_dates; do grep -q "\"$k\"" "$VAULT" && n=$((n+1)); done
  [ "$n" = "3" ] && ok "P the vault holds the three key values (names checked, values not read)" || note "P the vault holds $n of 3 key names (written when generated; check again after the next page load)"
else note "P no vault file at $VAULT"; fi
echo "  profile         $(pf profile)  dial +$(pf dial)  selector $(pf selector)  currency $(pf currency)"
echo "  sign-in gates   allow_leads $(pf allow_leads)  require_service $(pf require_service)  session days $(pf ttl_days)"
echo "  sessions        live customer sessions: $(pf sessions_live)"
if [ "$(pf profile)" = "uganda" ] && [ "$(pf dial)" = "256" ]; then ok "P the tenant profile resolves to uganda (+256): a national number is completed with Uganda's code"
else bad "P the tenant profile resolves to '$(pf profile)' (+$(pf dial)) — a Uganda number would be completed wrongly. The ONE approved configuration step, to run yourself and name in the deploy note:  docker exec -u $DB_OWNER -w $IN_CONTAINER $CONTAINER php tools/set_config.php --key tenant_profile --value uganda"; fi
docker exec -u "$DB_OWNER" "$CONTAINER" rm -rf "$RO" 2>/dev/null || true
...
if [ -n "$LOGIN_PHONE" ]; then
  ...
  if [ -n "$ADMIN_TOKEN" ]; then
    ...
    if [ "$HTTP_CODE" = "200" ]; then
      ...
      INUSE="$(jfield "$HTTP_BODY" in_use)"; ELIG="$(printf '%s' "$HTTP_BODY" | grep -o '"eligible":[a-z]*' | head -1 | cut -d: -f2)"; WHY="$(jfield "$HTTP_BODY" refused_because)"
      echo "  transport       wasender_configured=${WAS:-?}  evolution_configured=${EVO:-?}  in_use=${INUSE:-?}  dry_run_mode=${DRY:-?}"
      [ "${INUSE:-none}" != "none" ] && [ -n "${INUSE:-}" ] && ok "L1 a WhatsApp transport is in use ($INUSE) — the Phase 2 send path can carry a code" || bad "L1 no WhatsApp transport in use — app_send_otp will refuse"
      ELIGIBLE=1; if [ "${ELIG:-true}" = "false" ]; then ELIGIBLE=0; bad "L1 the sign-in gates would REFUSE this account (reason: ${WHY:-?}) — it gets the uniform answer and no code. See portal_login_allow_leads / portal_login_require_service; nothing is changed by this run"; else ok "L1 the account passes the sign-in gates"; fi
  ...
  ELIGIBLE="${ELIGIBLE:-1}"
  if [ "$ELIGIBLE" = "1" ]; then http POST "$PLUGIN_BASE?page=api&action=app_send_otp" "{\"phone\":\"$LOGIN_PHONE\"}"; else HTTP_CODE="skipped"; HTTP_BODY=""; fi
  ...
  elif [ "$HTTP_CODE" = "skipped" ]; then note "L2 not attempted: the gates would refuse this account (uniform answer, no code)"
  else
  ...
  if [ -z "$CODE" ]; then
    ...
    # Phase 2: a browser gets the session as an HttpOnly cookie and NO token in the JSON. This check announces
    # itself as a client (X-DishNet-Client) so the token also comes in the body for the Bearer checks below,
    # and it reads the cookie's flags from the response headers.
    http POST "$PLUGIN_BASE?page=api&action=app_verify_otp" "{\"phone\":\"$LOGIN_PHONE\",\"code\":\"$CODE\"}" "X-DishNet-Client: phase2-deploy-check"
    CTOK="$(jfield "$HTTP_BODY" token)"
    SC="$(printf '%s' "$HTTP_HEADERS" | tr -d '\r' | grep -i '^Set-Cookie: dn_customer_session=' | head -1)"
    CVAL="$(printf '%s' "$SC" | sed -E 's/^[^=]*=([^;]*).*/\1/')"
    if [ "$HTTP_CODE" = "200" ] && [ -n "$CTOK" ]; then
      ok "L4 app_verify_otp: 200, a customer token was issued (not printed)"
      [ -n "$SC" ] && ok "L4 the server set the session cookie (dn_customer_session)" || bad "L4 no Set-Cookie: dn_customer_session in the answer"
      printf '%s' "$SC" | grep -qi 'httponly' && ok "L4 …HttpOnly" || bad "L4 …the cookie is NOT HttpOnly"
      printf '%s' "$SC" | grep -qi 'samesite=lax' && ok "L4 …SameSite=Lax" || bad "L4 …the cookie is NOT SameSite=Lax"
      case "$PLUGIN_BASE" in https://*) printf '%s' "$SC" | grep -qi 'secure' && ok "L4 …Secure (the address is HTTPS)" || bad "L4 …the cookie is NOT Secure although the address is HTTPS — is X-Forwarded-Proto reaching PHP?";; *) note "L4 plain-http address: Secure not expected";; esac
      ...
      # Phase 2: the session is the HttpOnly cookie the server set at verification. A token in the URL is ignored.
      http GET "$PLUGIN_BASE?page=customer_portal&view=home" '' "Cookie: dn_customer_session=$CVAL"
      if [ "$HTTP_CODE" = "200" ] && printf '%s' "$HTTP_BODY" | grep -qi 'DishNet'; then ok "L6 the customer portal page with the session cookie: 200"
      elif [ "$HTTP_CODE" = "302" ]; then note "L6 the portal answered 302 with the cookie — consent still owed? (L5 records it; a second load would open)"; else bad "L6 customer_portal page with the cookie → $HTTP_CODE"; fi
      ENC_TOK="$(printf '%s' "$CTOK" | sed 's/+/%2B/g; s|/|%2F|g; s/=/%3D/g')"
      http GET "$PLUGIN_BASE?page=customer_portal&view=home&token=$ENC_TOK" ''
      [ "$HTTP_CODE" = "302" ] && ok "L6 the portal with the token in the URL and no cookie: 302 — the URL token is ignored" || bad "L6 the portal accepted or mishandled a URL token → $HTTP_CODE"
      http GET "$PLUGIN_BASE?page=customer_portal&view=home" ''
      [ "$HTTP_CODE" = "302" ] && ok "L6 the portal without a session: 302 to sign in" || bad "L6 the portal without a session → $HTTP_CODE"
      http GET "$PLUGIN_BASE?page=api&action=app_me&token=$ENC_TOK" ''
      [ "$HTTP_CODE" = "401" ] && ok "L6 app_me with ?token= only: 401 — the API accepts no URL token" || bad "L6 app_me accepted a URL token → $HTTP_CODE"
      unset ENC_TOK
      ...
      http GET "$PLUGIN_BASE?page=api&action=app_me" '' "$C"
      if [ "$HTTP_CODE" = "401" ] && [ "$(jfield "$HTTP_BODY" message)" = "Token revoked." ]; then ok "L7 the same token after logout: 401 Token revoked (the session row is revoked)"; else bad "L7 the token after logout → $HTTP_CODE $(jfield "$HTTP_BODY" message)"; fi
      http GET "$PLUGIN_BASE?page=customer_portal&view=home" '' "Cookie: dn_customer_session=$CVAL"
      [ "$HTTP_CODE" = "401" ] && ok "L7 the portal with the old cookie after logout: 401 (the session ended for the portal too)" || bad "L7 the portal still opened on a logged-out session → $HTTP_CODE"
      unset C CVAL SC
...
try { $st = $db->prepare("SELECT COUNT(*) FROM wa_messages WHERE body LIKE ?"); $st->execute(["%{$code}%"]); $hits += (int)$st->fetchColumn(); } catch (Throwable $e) {}
...
PHP
    ...
    [ "${HITS:-?}" = "0" ] && ok "L8 the code appears in no notification, queue, conversation, pending or audit row" || bad "L8 the code appears in ${HITS} stored row(s)"
    unset CODE CTOK
  fi
...
if [ "$FAIL" = "0" ]; then echo; echo "  Phase 2 smoke: PASSED. Send this LOG FILE back (not a copy of the terminal)."
else echo; echo "  Phase 2 smoke: $FAIL FAILED — send the log file; do not roll back on your own unless customers are affected."; fi
