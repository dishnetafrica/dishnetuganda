...
| Customer portal | `https://crm.dishnetuganda.com/crm/_plugins/dishnet-hybrid-sudan/public.php?page=customer_login` — the DishNet portal sign-in (26 Sep 2026; decision 8 of the customer-login remediation plan). It was uCRM's own client-zone login `/crm/login` from 25 Sep, and the South Sudan plugin URL before that. Never the bare `/crm`: the server in front of uCRM redirects it to `/crm/` on its own port, `:8443`, which has a self-signed certificate |
...
| Portal deep-link | DONE 26 Sep 2026: every "Customer Login" (176 links on 57 pages) opens `public.php?page=customer_login`, which sends a customer with a live session straight to the portal. `verify-site.sh` allows exactly that one portal URL. A short address (`/customer-login`, one Traefik file) is optional and separate |
